...
func (requester requester) request(endpoint string, httpVerb string, requestParams map[string]string, body map[string]interface{}, headers map[string]string) (string, error) {
...
func getHeader(m map[string]string) string {
	b := new(bytes.Buffer)
	for key := range m {
		fmt.Fprintf(b, "%s", key)
	}
